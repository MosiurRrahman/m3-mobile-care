...
CSS;
   ...
   * @return string
   */
  public static function compressAndStoreImage($file, $folder, $disk = 'public')
  {
    try {
      ...
      // Resize image if it exceeds max dimension (e.g., 1200px)
      $maxDimension = 1200;
      ...
      if ($mime === 'image/jpeg' || $mime === 'image/jpg') {
        $success = imagejpeg($image, null, 75); // Quality 75%
      } elseif ($mime === 'image/png') {
        $success = imagepng($image, null, 6); // Compression level 6 (0-9)
      } elseif ($mime === 'image/gif') {
        $success = imagegif($image, null);
      } elseif ($mime === 'image/webp') {
        $success = imagewebp($image, null, 75); // Quality 75%
      }

      $compressedContent = ob_get_clean();
      imagedestroy($image);

      if ($success && $compressedContent !== false) {
        $filename = Str::random(40) . '.' . $extension;
        $targetPath = $folder . '/' . $filename;
        Storage::disk($disk)->put($targetPath, $compressedContent);
        return $targetPath;
      }
    } catch (\Throwable $e) {
      // Log error or ignore and let it fallback to standard store
      report($e);
    }

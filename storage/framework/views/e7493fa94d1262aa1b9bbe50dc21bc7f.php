        ...
        <div class="card border-0 shadow-sm h-100">
            ...
            <div class="tab-content card-body pt-3" id="partnerActionTabContent">
                ...
                <div class="tab-pane fade show active" id="deposit-tab-pane" role="tabpanel" aria-labelledby="deposit-tab" tabindex="0">
                    <form action="<?php echo e(route('admin.partner-ledger.deposit')); ?>" method="POST" onsubmit="return confirm('Are you sure you want to record this investment / deposit?')">
                        <?php echo csrf_field(); ?>
                        <div class="mb-3">
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            ...
                            <td style="max-width: 250px; white-space: normal;"><?php echo e($entry->description); ?></td>
                            <td><span class="text-muted small"><?php echo e($entry->creator ? $entry->creator->name : 'System'); ?></span></td>
                            <td class="text-end">
                                <button type="button" class="btn btn-sm btn-icon btn-label-primary me-1" data-bs-toggle="modal" data-bs-target="#editEntryModal<?php echo e($entry->id); ?>" title="Edit Entry">
                                    <i class="ti tabler-pencil fs-6"></i>
                                </button>
                                <form action="<?php echo e(route('admin.partner-ledger.destroy', $entry->id)); ?>" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this ledger entry? Partner balances will be adjusted automatically.')">
                                    <?php echo csrf_field(); ?>
                                ...
                                <!-- Edit Modal -->
                                <div class="modal fade" id="editEntryModal<?php echo e($entry->id); ?>" tabindex="-1" aria-hidden="true" style="white-space: normal;">
                                    <div class="modal-dialog modal-dialog-centered text-start">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title fw-bold">Edit Ledger Entry #<?php echo e($entry->id); ?></h5>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <form action="<?php echo e(route('admin.partner-ledger.update', $entry->id)); ?>" method="POST">
                                                <?php echo csrf_field(); ?>
                                                ...
                                                <div class="modal-body">
                                                    <div class="mb-3">
                                                        ...
                                                        <select name="partner_name" class="form-select" required>
                                                            <option value="Monowar Munna" <?php echo e($entry->partner_name === 'Monowar Munna' ? 'selected' : ''); ?>>Monowar Munna</option>
                                                            <option value="Munna Raihan" <?php echo e($entry->partner_name === 'Munna Raihan' ? 'selected' : ''); ?>>Munna Raihan</option>
                                                            <option value="Mosiur" <?php echo e($entry->partner_name === 'Mosiur' ? 'selected' : ''); ?>>Mosiur</option>
                                                        </select>
                                                    </div>
                                                    <div class="row mb-3">
                                                        <div class="col-6">
                                                            ...
                                                            <select name="account_type" class="form-select" required>
                                                                <option value="capital" <?php echo e($entry->account_type === 'capital' ? 'selected' : ''); ?>>Capital Account (মূলধন)</option>
                                                                <option value="profit" <?php echo e($entry->account_type === 'profit' ? 'selected' : ''); ?>>Profit Share Account (লভ্যাংশ)</option>
                                                            </select>
                                                        </div>
                                                        <div class="col-6">
                                                            <label class="form-label fw-semibold">Entry Type</label>
                                                            <select name="type" class="form-select" required>
                                                                <option value="credit" <?php echo e($entry->type === 'credit' ? 'selected' : ''); ?>>Credit (Deposit / Investment)</option>
                                                                <option value="debit" <?php echo e($entry->type === 'debit' ? 'selected' : ''); ?>>Debit (Withdrawal / Payout)</option>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label class="form-label fw-semibold">Amount (BDT)</label>
                                                        <input type="number" name="amount" step="0.01" min="0.01" class="form-control" value="<?php echo e($entry->amount); ?>" required>
                                                    </div>
                                                    <div class="mb-3">
                                                        <label class="form-label fw-semibold">Description / Notes</label>
                                                        <textarea name="description" rows="2" class="form-control"><?php echo e($entry->description); ?></textarea>
                                                    </div>
                                                </div>
...
<?php echo $__env->make('layouts/contentNavbarLayout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\laragon\www\m3-mobile-care\resources\views/partner_ledger/index.blade.php ENDPATH**/ ?>
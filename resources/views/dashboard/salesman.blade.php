...
@section('title', 'Salesman Dashboard - M3 Mobile Care')
            ...
            <div class="card-body p-4 p-md-5">
                <div class="row align-items-center">
                    ...
                    <div class="col-md-4 text-md-end mt-3 mt-md-0">
                        <a href="{{ route('admin.pos.index') }}" class="btn btn-white bg-white text-primary fw-bold px-4 py-2.5 shadow-sm"><i class="ti tabler-device-laptop me-1"></i>Open POS Terminal</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="alert alert-warning border-0 shadow-sm d-flex align-items-center justify-content-between p-3" role="alert">
            <div class="d-flex align-items-center">
                ...
                <div>
                    ...
                    <p class="mb-0 small">There are <strong>{{ $accessoriesAlertCount }}</strong> mobile accessories in inventory that are running low on stock levels.</p>
                </div>
            </div>
            <a href="{{ route('admin.inventory.accessories') }}" class="btn btn-warning btn-sm fw-bold">Manage Stock</a>
        </div>
    </div>
    @endif
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <h3 class="card-title fw-extrabold mb-0 text-primary">{{ $mySalesTodayCount }}</h3>
                </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <h3 class="card-title fw-extrabold mb-0 text-success">{{ number_format($mySalesTodayRevenue, 2) }} BDT</h3>
                </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <h3 class="card-title fw-extrabold mb-0 text-info">{{ $activeAccessoriesCount }}</h3>
                </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body d-flex align-items-center justify-content-between">
                <div>
                    ...
                    <h3 class="card-title fw-extrabold mb-0 text-danger">{{ $accessoriesAlertCount }}</h3>
                </div>
    ...
    <div class="col-12">
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('admin.pos.index') }}" class="btn btn-primary btn-sm"><i class="ti tabler-plus me-1"></i>New Sale Checkout</a>
            </div>
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><span class="fw-bold text-dark">{{ $sale->invoice_no }}</span></td>
                            <td>
                                <span class="fw-semibold">{{ $sale->customer ? $sale->customer->name : 'Walk-in Customer' }}</span>
                                @if($sale->customer)
                                    <div class="text-muted small">{{ $sale->customer->phone }}</div>
                                @endif
                            </td>
                            <td>{{ number_format($sale->total_amount, 2) }} BDT</td>
                            <td>{{ number_format($sale->discount, 2) }} BDT</td>
                            <td><span class="fw-bold text-success">{{ number_format($sale->payable_amount, 2) }} BDT</span></td>
                            <td><span class="badge bg-label-primary">{{ $sale->payment_method }}</span></td>
                            <td><span class="small text-muted">{{ $sale->created_at->format('M d, Y H:i A') }}</span></td>
                            <td class="text-end">
                                <a href="{{ route('admin.pos.invoice', $sale->id) }}" target="_blank" class="btn btn-icon btn-sm btn-outline-info" title="Print Invoice"><i class="ti tabler-printer"></i></a>
                            </td>
                        </tr>
                        @empty
                        <tr>

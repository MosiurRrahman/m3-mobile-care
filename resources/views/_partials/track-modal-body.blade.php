    ...
    <div class="col-lg-5">
        <div class="glass-card p-4 h-100 bg-white bg-opacity-5" style="border-radius: 16px;">
            ...
            <div class="d-flex flex-column gap-3">
                <div class="border-bottom border-white border-opacity-5 pb-3">
                ...
                <div class="border-bottom border-white border-opacity-5 pb-3">
                    ...
                    <span class="value-text d-flex align-items-center">
                        <i class="ti tabler-device-mobile text-primary me-2"></i>{{ $repair->device_brand }} {{ $repair->device_model }}
                    </span>
                </div>
                <div class="border-bottom border-white border-opacity-5 pb-3">
                ...
                <div>
                    ...
                    <span class="text-warning fw-bold"><i class="ti tabler-calendar-clock me-2"></i>{{ \Carbon\Carbon::parse($repair->expected_delivery_date)->format('M d, Y') }}</span>
                </div>
                @endif
            </div>
            ...
            @php
                ...
                $steps = [
                    'pending' => [
                        ...
                        'Desc' => 'Ticket #' . $repair->ticket_id . ' created for ' . $repair->device_brand . ' ' . $repair->device_model . ' & logged in database.', 
                        'Icon' => 'tabler-ticket'
                    ],
                    'diagnosing' => [
                        'Title' => 'Diagnostic Assessment', 
                        'Desc' => 'Technician inspecting reported issue: "' . $repair->issue_description . '"', 
                        'Icon' => 'tabler-zoom-check'
                    ],
                    'waiting_for_approval' => [
                        'Title' => 'Pending Approval', 
                        'Desc' => 'Estimated cost ' . number_format($repair->estimated_cost, 0) . ' BDT. Awaiting customer consent.', 
                        'Icon' => 'tabler-clock'
                    ],
                    'repairing' => [
                        'Title' => 'Active Repair Room', 
                        'Desc' => $techNotes ? $techNotes : 'Device is actively under repair in our lab by technician.', 
                        'Icon' => 'tabler-tool'
                    ],
                    'quality_check' => [
                        'Title' => 'Quality Checks', 
                        'Desc' => 'Testing touch responses, camera focus, display & charging performance.', 
                        'Icon' => 'tabler-shield-check'
                    ],
                    'completed' => [
                        'Title' => 'Ready for Pickup', 
                        'Desc' => 'Successfully restored! Device is ready for collection' . ($repair->expected_delivery_date ? ' (Expected: ' . \Carbon\Carbon::parse($repair->expected_delivery_date)->format('M d, Y') . ')' : '') . '.', 
                        'Icon' => 'tabler-checkbox'
                    ],
                    'delivered' => [
                        'Title' => 'Closed / Handover', 
                        'Desc' => 'Settled and delivered back to customer (' . ($repair->customer ? $repair->customer->name : 'Walk-in') . ').', 
                        'Icon' => 'tabler-archive'
                    ]
                ];
                ...
                if ($status == 'cancelled') {
                    ...
                    // Only show status steps up to current complete/active status

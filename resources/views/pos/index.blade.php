...
@section('title', 'Sales POS Terminal - M3 Mobile Care')
                ...
                <div class="row row-cols-1 row-cols-sm-2 row-cols-xl-3 g-4" id="products-catalog-list" style="max-height: 600px; overflow-y: auto; padding-right: 5px;">
                    ...
                    <div class="col product-card-wrapper mb-2" data-id="{{ $item->id }}" data-name="{{ $item->name }}" data-sku="{{ $item->sku }}" data-barcode="{{ $item->barcode }}" data-price="{{ $item->sale_price }}" data-min-price="{{ $item->effective_min_price }}" data-qty="{{ $item->quantity }}" data-category="{{ $item->categoryRelation ? $item->categoryRelation->name : $item->category }}" data-brand="{{ $item->brand ?: 'Generic' }}" data-cost-price="{{ $item->purchase_price }}" data-description="{{ $item->description }}" data-variants="{{ json_encode($item->variants) }}" data-image="{{ !empty($item->images) && is_array($item->images) && isset($item->images[0]) ? asset('storage/' . $item->images[0]) : '' }}" data-qty-alert="{{ $item->alert_quantity }}">
                        <div class="product-pos-card shadow-xs {{ $item->quantity <= 0 ? 'product-out-of-stock' : '' }}">
                            ...
                            <button class="btn btn-light btn-xs quick-view-btn position-absolute shadow-xs" data-id="{{ $item->id }}" style="top: 10px; right: 10px; z-index: 10; width: 32px; height: 32px; border-radius: 50%; padding: 0; background: rgba(255, 255, 255, 0.9); border: 1px solid #eef2f6;" title="Quick View">
                            ...
                            <div class="product-info-container mb-3">
                                <h6 class="fw-bold text-dark mb-1 text-wrap text-start" style="font-size: 0.85rem; line-height: 1.35; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; height: 2.3rem;" title="{{ $item->name }}">{{ $item->name }}</h6>
                                <span class="text-muted d-block text-start mb-2" style="font-size: 0.72rem;">{{ $item->sku }} · <span class="fw-semibold text-info">{{ str_replace('_', ' ', ucfirst($item->type)) }}</span></span>
                            </div>
                            
                            <!-- Bottom row -->
                            <div class="d-flex align-items-end justify-content-between">
                                <div class="text-start">
                                    <span class="fw-extrabold text-dark d-block mb-1" style="font-size: 1rem; font-weight: 800;">{{ number_format($item->sale_price, 2) }} BDT</span>
                                    @if($item->quantity <= $item->alert_quantity)
                                        <span class="stock-pill-danger">{{ $item->quantity }} Piece</span>
                                    @else
                                        <span class="stock-pill-success">{{ $item->quantity }} Piece</span>
                                    @endif
                                </div>
                                
                                <button class="btn btn-teal-accent add-to-cart-btn position-relative" data-id="{{ $item->id }}" style="width: 40px; height: 40px; border-radius: 10px; padding: 0;">
                                    <i class="ti tabler-shopping-cart fs-4"></i>
                                    <!-- Dynamic Qty Badge -->
                                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger cart-item-qty-badge d-none" id="qty-badge-{{ $item->id }}" style="font-size: 0.65rem; padding: 0.2rem 0.4rem; z-index: 5;">
                                        0
                                    </span>
                                </button>
                            </div>
